...
class Conference:
    ...
    def _find_conference_teams(self, conference_abbreviation, year):
        ...
        if not page:
            ...
            output = ("Likely exeeded rate limite of website: %s" % url)
...
class Conferences:
    ...
    def _pull_conference_page(self, year):
        ...
        while status:
            try:
                time.sleep(5)
                return pq(f'https://www.sports-reference.com/cbb/seasons/men/{year}.html')
                status = False
    ...
    def _find_conferences(self, year):
        ...
        if not page:
            output = ("Likely exeeded rate limite of website - Edit code section that follows line 186: %s" % (CONFERENCES_URL % year))
            raise ValueError(output)
